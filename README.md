...
```mermaid
flowchart LR
    zmw_cat_snack_dispenser["ZmwCatSnackDispenser<br/><code>zmw_cat_snack_dispenser</code>"]
    zmw_contactmon["ZmwContactmon<br/><code>zmw_contactmon</code>"]
    zmw_cronenbergs["ZmwCronenbergs<br/><i>no public interface</i>"]
    zmw_doorman["ZmwDoorman<br/><code>zmw_doorman</code>"]
    zmw_heating["ZmwHeating<br/><code>zmw_heating</code>"]
    zmw_homeboard["ZmwHomeboard<br/><code>zmw_homeboard</code>"]
    zmw_lights["ZmwLights<br/><code>zmw_lights</code>"]
    zmw_reolink_cams["ZmwReolinkCams<br/><code>zmw_reolink_cams</code>"]
    zmw_sensormon["ZmwSensormon<br/><code>zmw_sensormon</code>"]
    zmw_shelly_plug["ZmwShellyPlug<br/><code>zmw_shelly_plug</code>"]
    zmw_sonos_ctrl["ZmwSonosCtrl<br/><code>zmw_sonos_ctrl</code>"]
    zmw_speaker_announce["ZmwSpeakerAnnounce<br/><code>zmw_speaker_announce</code>"]
    zmw_speech_to_text["ZmwSpeechToText<br/><code>zmw_speech_to_text</code>"]
    zmw_spotify["ZmwSpotify<br/><code>zmw_spotify</code>"]
    zmw_telegram["ZmwTelegram<br/><code>zmw_telegram</code>"]
    zmw_text_to_speech["ZmwTextToSpeech<br/><code>zmw_text_to_speech</code>"]
    zmw_unifi_clientmon["ZmwUnifiClientmon<br/><code>zmw_unifi_clientmon</code>"]
    zmw_visitor_detect["ZmwVisitorDetect<br/><code>zmw_visitor_detect</code>"]
    zmw_whatsapp["ZmwWhatsapp<br/><code>zmw_whatsapp</code>"]
    zigbee2mqtt(["zigbee2mqtt<br/><i>Zigbee2MQTT daemon</i>"])

    zmw_cat_snack_dispenser <--> zigbee2mqtt
    zmw_cat_snack_dispenser <--> zmw_telegram
    zigbee2mqtt --> zmw_contactmon
    zmw_contactmon --> zmw_speaker_announce
    zmw_contactmon --> zmw_telegram
    zmw_contactmon --> zmw_whatsapp
    zmw_cronenbergs <-.-> zigbee2mqtt
    zmw_cronenbergs -.-> zmw_speaker_announce
    zmw_cronenbergs -.-> zmw_telegram
    zmw_doorman <--> zmw_contactmon
    zmw_doorman <--> zmw_reolink_cams
    zmw_doorman --> zmw_speaker_announce
    zmw_doorman <--> zmw_telegram
    ...
    zmw_shelly_plug --> zmw_sensormon
    zmw_sonos_ctrl <--> zmw_spotify
    ...
    class zigbee2mqtt external;
    class zmw_cronenbergs passive;
    classDef external fill:#fde68a,stroke:#b45309,color:#111;
    classDef passive fill:#e5e7eb,stroke:#9ca3af,color:#6b7280,stroke-dasharray:4 3;
```
